...
//     cuando difiere del nombre legal).
//   - Búsqueda automática (botón "🔍 Buscar") vía /api/empresas/[id]/links/sync.
//     Hoy el endpoint devuelve 501 hasta que se completen las credenciales
//     Microsoft Graph + Notion API; cuando llegue, el matcher poblará URLs.
    ...
    <div>
        ...
        <div className="space-y-1.5">
          {initial.oneDriveUrl ? (
            ...
            <div className="flex items-center gap-2 px-2.5 py-1.5 border border-dashed border-wr-border rounded text-[10px] text-wr-hint italic">
              <span>📁</span> OneDrive — sin link
            </div>
          )}
          {initial.notionUrl ? (
            <a
              href={initial.notionUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-2.5 py-1.5 bg-wr-surface2 border border-wr-border rounded text-xs text-wr-text hover:border-wr-blue/50 hover:bg-wr-blue/5 transition-colors"
            >
            ...
            <div className="flex items-center gap-2 px-2.5 py-1.5 border border-dashed border-wr-border rounded text-[10px] text-wr-hint italic">
              <span>📓</span> Notion — sin link
            </div>
          )}
          {initial.nombreComercial && (
            <div className="text-[10px] text-wr-hint pt-1">
              Alias: <span className="text-wr-muted">{initial.nombreComercial}</span>
            </div>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-wr-hint block mb-0.5">📁 OneDrive URL</label>
            <input
              type="url"
              value={oneDriveUrl}
          ...
          <div>
            <label className="text-[10px] text-wr-hint block mb-0.5">📓 Notion URL</label>
            <input
              type="url"
              value={notionUrl}
