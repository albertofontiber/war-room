...
export default function PortalForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      // Endpoint anti-enumeración: siempre 200, exista o no el email.
      // Solo nos importa que se complete; no usamos el resultado.
      ...
      // Aunque falle la red, mostramos el mismo mensaje genérico — no
      // queremos que el usuario sepa si su email está registrado o no.
    } finally {
      setLoading(false);
      setSubmitted(true);
    }
          ...
          {submitted ? (
            <>
              ...
              <p className="text-wr-muted text-sm mb-6">
                Si <span className="text-wr-text">{email}</span> está registrado,
                en breve recibirás un email con instrucciones para restablecer
                tu contraseña. El enlace caduca en 24 horas.
              </p>
              <Link
                href="/portal/login"
            ...
            <>
              ...
              <p className="text-wr-muted text-sm mb-6">
                Introduce tu email y te enviaremos un link para elegir una
                contraseña nueva.
              </p>
              ...
              <Link
                href="/portal/login"
                className="block text-center text-wr-blue hover:underline text-xs mt-6"
              >
                ← Volver al login
              </Link>
            </>
