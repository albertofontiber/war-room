...
- **Consistencia tipográfica + pulido UI móvil** (PRs #138-#139, 2026-05-29): fix de raíz de fuente — `tailwind.config.ts` usaba el literal `"Inter"`/`"JetBrains Mono"` en `fontFamily`, que NO casa con la familia hasheada de `next/font`, así que la fuente cargada quedaba huérfana y cada dispositivo caía a su fuente de sistema (Segoe UI en portátil, San Francisco en iOS) → tipografía distinta entre móvil y escritorio. Ahora `fontFamily` usa `var(--font-sans)`/`var(--font-mono)` y se carga JetBrains Mono. Además, pulido móvil: chip "Posible adq." ya no se parte en 2 líneas (Señales M&A), regla anti-zoom de iOS hecha quirúrgica (`select` e `input[type=date]` recuperan su tamaño denso), métricas de grupo visibles en móvil (Grupos), "+ Lead"/"Excel" en la misma línea (Pipeline), wordmark móvil "WR" + buscador más ancho, y filtro "Sin actividad ≥ días" consistente. ✅
...
- **Fuente cargada vs literal en Tailwind** (PR #138): `fontFamily` en `tailwind.config.ts` debe referenciar las CSS vars de `next/font` (`var(--font-sans)`, `var(--font-mono)`), NUNCA el literal `"Inter"`/`"JetBrains Mono"`. next/font genera una familia hasheada (`__Inter_xxx`); el literal no casa con ella y la fuente cargada queda huérfana → cada dispositivo cae a su fuente de sistema (síntoma: tipografía distinta entre móvil y escritorio). `<html>` lleva `${inter.variable} ${jetbrainsMono.variable}` (layout.tsx) y el preflight de Tailwind aplica `fontFamily.sans` al `html`.
- **Anti-zoom iOS (16px) solo en inputs de texto** (PRs #138-#139): la regla `@media (max-width:639px)` en `globals.css` fuerza `font-size:16px` para evitar el zoom-in de iOS al enfocar inputs que abren teclado. EXCLUYE `select` e `input[type=date]` (+ checkbox/radio/range/color/file): abren picker nativo, no teclado → no hacen zoom, así que conservan su tamaño denso (`text-[10px]`) y quedan consistentes con escritorio. NO volver a meter `select`/`date` en la regla. Para un input numérico que sí queda a 16px (ej. filtro "días"), poner la unidad como etiqueta estática aparte, no como placeholder del input.
